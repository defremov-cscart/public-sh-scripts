...
echo -n "Enter destination server address "

read -p 'Destination server IP or domain name: ' destserver
...
echo -n "Check your data. User - $adduser, destination server - $destserver. Continue? (Y/n) "
read item
  case "$item" in
...
sudo sh -c "echo '$adduser      ALL=(ALL) NOPASSWD: ALL' >> /etc/sudoers.d/admin"
EOF
